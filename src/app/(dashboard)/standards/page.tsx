...
// Mock project data — showing which standards already have active projects
const activeProjects: Record<string, { name: string; score: number }> = {
  ISO27001: { name: "ISO 27001 Certification 2026", score: 68 },
  ISO9001: { name: "Quality Management Programme", score: 84 },
  ISO45001: { name: "OH&S Certification Renewal", score: 91 },
};
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4">
        ...
        <div>
          <p className="text-sm font-medium text-blue-900">
            Real ISO control references
          </p>
          <p className="text-sm text-blue-700 mt-0.5">
            All standards include real clause numbers, control references, and
            descriptions based on the published ISO standards.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
        {ISO_STANDARDS.map((standard) => {
          ...
          const totalClauseControls = standard.clauses.reduce(
            (sum, c) => sum + c.controls.length,
            0
          );
          ...
          return (
            <Card
              key={standard.code}
              className={`border ${meta.borderColor} flex flex-col`}
            >
              <CardHeader>
                <div className="flex items-start justify-between gap-3">
                  <div
                    className={`size-10 rounded-lg ${meta.bgColor} flex items-center justify-center shrink-0`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <Badge
                      variant="outline"
                      className={`mt-1 text-xs ${meta.color} border-current/30`}
                    >
                      {meta.category}
                    </Badge>
                  </div>
              ...
              <CardContent className="flex flex-col flex-1 gap-4">
                  ...
                  <ul className="space-y-1">
                    ...
                    {standard.clauses.length > 4 && (
                      <li className="text-xs text-muted-foreground pl-5">
                        +{standard.clauses.length - 4} more clauses
                      </li>
                    )}
                  </ul>
                </div>
                ...
                {project && (
                  <div className="rounded-lg bg-muted/60 p-3 text-xs">
                    ...
                    <p className={`font-semibold mt-1 ${meta.color}`}>
                      {project.score}% compliant
                    </p>
                  </div>
                )}
